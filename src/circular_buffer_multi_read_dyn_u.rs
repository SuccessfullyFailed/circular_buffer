#[cfg(test)]
mod tests {
	use crate::{ CircularBufferMultiReadDyn, ReadCursor };
	use std::time::{ Duration, Instant };
	
	

	const TEST_CAPACITY:usize = 8;
	fn get_test_buffer() -> CircularBufferMultiReadDyn<i32> {
		CircularBufferMultiReadDyn::new(TEST_CAPACITY)
	}
	...
	fn test_multi_read_stress_test_large_cycles() {
		...
		let mut buffer:CircularBufferMultiReadDyn<i32> = CircularBufferMultiReadDyn::new(1024);
		let cursor_a:ReadCursor = buffer.create_read_cursor();
		let cursor_b:ReadCursor = buffer.create_read_cursor();
	...
	fn test_multi_read_performance_timing() {
		...
		let mut buffer:CircularBufferMultiReadDyn<i32> = CircularBufferMultiReadDyn::new(2048);
		let cursor_a:ReadCursor = buffer.create_read_cursor();
		let cursor_b:ReadCursor = buffer.create_read_cursor();
	...
	fn test_multi_read_complex_read_altering() {
		let mut buffer:CircularBufferMultiReadDyn<i32> = CircularBufferMultiReadDyn::new(350);
